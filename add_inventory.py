...
def add_inventory_widgets_to_frame(frame):
    ...
    def delete_row():
        ...
        if selected_items:
            for item in selected_items:
                ...
                # Delete rows from recently_added and mediator tables
                query = "DELETE FROM recently_added, mediator USING recently_added JOIN mediator ON recently_added.product_id = mediator.product_id WHERE recently_added.product = %s AND recently_added.colour = %s AND recently_added.size = %s AND recently_added.qty = %s AND recently_added.rate = %s AND recently_added.product_id = %s"
    ...
    def edit_record():
        ...
        e_0.configure(state='readonly')
        e_1.configure(state='readonly')
        e_2.configure(state='readonly')
        e_3.configure(state='readonly')
    ...
    def update_record():
        ...
        updated_product = e_0.get() + ", " + e_1.get()
        updated_colour = e_2.get()
        updated_size = e_3.get()
        updated_qty = int(e_4.get())
        updated_rate = int(e_5.get())

        tree.item(selected,
                  values=(updated_product, updated_colour, updated_size, updated_qty, updated_rate, values[5]))

        db = mysql.connector.connect(host='localhost', user='root', password='hrjagtap@1', database='inventory')
        cursor = db.cursor()

        query = "UPDATE recently_added SET product = %s, colour = %s, size = %s, qty = %s, rate = %s WHERE product_id = %s"
        values = (updated_product, updated_colour, updated_size, updated_qty, updated_rate, values[5])
        cursor.execute(query, values)
        db.commit()
